...
`max_file_size_bytes` 用来限制参与扫描和同步的单个文件大小。JSONL 超过上限时，ccs 会跳过该文件，并在日志中输出 warning，包含文件路径、实际大小和配置上限。
...
| 按键 | 作用 |
|------|------|
| `↑` `↓` | 移动光标（循环），`PageUp`/`PageDown` 翻页，`Home`/`End` 跳转 |
| `→` 或 `Enter` | 进入选中项（下一级） |
| `←` | 返回上一级（项目列表 ← 会话列表 ← 操作菜单） |
| 直接打字 | 过滤当前列表（大小写不敏感），`Backspace` 删除 |
| `Esc` / `Ctrl-C` | 取消：顶层菜单退出程序，操作菜单返回上一级 |
- 选择项目后进入该项目的会话列表
